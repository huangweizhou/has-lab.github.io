...
本文提出将计算节点地址到FAM内存地址的映射信息缓存在计算节点本地内存，以加速二位地址转换。

具体实现：如图4①②所示，本文分别在计算节点的内存控制器中实现了FAM translator和FAM translation cache（类似于现有MMU内部的TLB），每次在进行FAM内存访问时，FAM translator会先去查询FAM translation cache，如果命中，意味着获取到FAM内存地址，接着执行图4中第③步，即STU读取ACM以检查访问权限；入果cache未命中，FAM translator会请求STU进行FAM内存页表访问（图4④），并更新FAM translation cache（图4⑤）。
