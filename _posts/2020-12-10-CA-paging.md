...
如图4所示，
当一个VMA被第一次访问时，发生第一次page fault，为了避免随机分配物理内存段导致的碎片化问题，CA paging用VMA的大小作为key查询contiguity map, 找到最合适的的块分配给当前发生fault的区域。**CA paing并不会为整个VMA分配物理内存，而是仅为当前发生fault的区域分配相应的内存。**
