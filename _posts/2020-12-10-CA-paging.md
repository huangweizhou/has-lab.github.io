...
当同一个VMA在下次发生fault时，就可以通过已经记录的offset和当前发生fault的虚拟内存地址计算出目标物理页，再通过检查目标物理页的反向映射信息判断其是否空闲，若空闲，则这段大页映射的尺寸可以增加。如果发现目标物理页被占用（图5中黑色的页），并且这个page fault是大尺寸VMA（大于4KB）触发的，那么CA paging会为剩余发生fault区域重新执行大页分配，并记录新的offset。
