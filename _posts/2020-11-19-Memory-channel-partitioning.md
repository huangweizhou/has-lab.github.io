...
<img src="../images/memory-partitioning-row-buffer.png" width="65%" height="65%" />
...
作者提出了两个设计，内存通道分区机制，和将此机制与调度器结合。如下图所示，其中MPK和RBH分别是
Misses Per Kilo Instruction和Row-Buffer Hit Rate。

<center>

<img src="../images/memory-partitioning-MCP.png" width="45%" height="65%" />
...
衡量指标:Row-Buffer Hit Rate (RBH)

为了计算一个应用的固有（即单独运行时）的row-buffer-hit，本文为每个内存bank使用了一个per-core shadow row-buffer（额外的硬件，如下表所示） index，以记录一个应用独立运行时将会出现在row-buffer中的row



<center>
表1 硬件设计
<img src="../images/memory-partitioning-hw.png" width="900%" height="150%" />
...
衡量指标:Misses Per Kilo Instruction (MPKI）
